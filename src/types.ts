...
export type PlatformLogsKit_Config = {
  defaultViewPlan: Extract<LogTailUiPlan, { kind: "view" }>;
...
export function logTailUiDefaultViewPlan(
...
export function platformLogsKitFindSource(
  config: PlatformLogsKit_Config,
  sourceId: string,
): PlatformLogsKit_SourceConfig | undefined {
  return config.sources.find((source) => source.id === sourceId);
}

/** Returns a configured scope for a source or undefined. */
export function platformLogsKitFindScope(
  source: PlatformLogsKit_SourceConfig,
  scopeId: string,
): PlatformLogsKit_ScopeConfig | undefined {
