...
import {
  logTailUiDescribeSource,
  logTailUiSourceLabel,
} from "./resolve-log-targets.js";
import {
...
export function logTailUiBuildPreviewPlainText(
  ...
  return [
    ...
    plan.timeoutSeconds !== ""
      ...
      : "Timeout: (none)",
    `Inspection: errorsOnly=${String(plan.inspection.errorsOnly)} substring=${
      plan.inspection.substring || "(none)"
    } jsonLines=${String(plan.inspection.jsonLines)}`,
    "",
...
export function logTailUiBuildSummaryLine(
  config: PlatformLogsKit_Config,
  plan: LogTailUiPlan,
): string {
  if (plan.kind === "list-runs") {
