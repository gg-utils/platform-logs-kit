...
/** View-only log-tail plan shape shared by shortcut helper functions. */
type LogTailUiViewPlan = Extract<LogTailUiPlan, { kind: "view" }>;

function platformLogsKitShellQuote(value: string): string {
  if (value.length === 0) {
    return "''";
  }
...
function shortcutMatchesPlan(
  shortcut: PlatformLogsKit_Config["shortcuts"][number],
  plan: LogTailUiViewPlan,
): boolean {
...
export function logTailUiFormatNpmRunShortcut(
  ...
  const shortcut = config.shortcuts.find((candidate) =>
    shortcutMatchesPlan(candidate, plan),
  );
  return shortcut?.npmCommand ?? null;
}

/** Formats `npm run <script>` with forwarded argv appended after `--`. */
export function logTailUiFormatNpmRunWithForwardedArgs(
  npmScript: string,
...
export function logTailUiFormatTsxCommand(
