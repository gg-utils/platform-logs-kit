...
export function logTailUiBuildSingleFileSourceArgv(
...
export function logTailUiBuildScopedRunSourceArgv(
  config: PlatformLogsKit_Config,
  plan: Extract<LogTailUiPlan, { kind: "view" }>,
  ...
  if (scope === undefined) {
    throw new Error(
      `Unknown log scope '${plan.scope}' for source '${source.id}'.`,
    );
  }
...
export function logTailUiBuildCanonicalArgv(
  config: PlatformLogsKit_Config,
  plan: LogTailUiPlan,
): PlatformLogsKit_CanonicalArgv {
