...
export function logTailUiLineLooksLikeError(line: string): boolean {
  return ERRORISH.test(line);
}

/** Attempts to parse a log line as JSON. */
export function logTailUiTryParseJsonLine(
  line: string,
...
export function logTailUiLineMatchesInspection(
  line: string,
...
export function logTailUiFilterLines(
  lines: readonly string[],
  inspection: LogTailUiInspection,
): string[] {
  return lines.filter((line) =>
    logTailUiLineMatchesInspection(line, inspection),
  );
}

/** Counts how many lines look like errors. */
export function logTailUiCountErrors(lines: readonly string[]): number {
  return lines.reduce(
    (count, line) => count + (logTailUiLineLooksLikeError(line) ? 1 : 0),
    0,
  );
}
