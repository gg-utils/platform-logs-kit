...
import { logTail_resolveLatestLogFile, logTail_resolveNthNewestMatchingFile, } from "@gg-utils/platform-logs-core/discover-log-files";
import { platformLogsKitFindScope, platformLogsKitFindSource, } from "./types.js";
/** Pushes `absolutePath` only when it exists and is a regular file. */
function appendIfFile(files, absolutePath) {
    try {
...
function resolveSingleFileHistory(options) {
    ...
    const resolved = logTail_resolveLatestLogFile({
        ...
        latestSymlinkPath: path.join(options.projectRoot, history.latestSymlinkPath),
    });
...
function resolveRunFile(options) {
    ...
    if (runIndex === 1) {
        return logTail_resolveLatestLogFile({
            ...
            latestSymlinkPath: path.join(options.projectRoot, history.runLatestSymlinkPath),
        });
    }
...
export function logTailUiResolveHistoryFiles(config, projectRoot, plan) {
...
export function logTailUiDescribeSource(config, plan) {
    const source = platformLogsKitFindSource(config, plan.source);
    if (source === undefined) {
        return plan.source;
    }
...
export function logTailUiSourceLabel(config, sourceId) {
    return platformLogsKitFindSource(config, sourceId)?.compactLabel ?? sourceId;
}
//# sourceMappingURL=resolve-log-targets.js.map